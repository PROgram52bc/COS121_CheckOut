...
int main(int argc, char* argv[], char* envp[]) {
    ...
    for ( int minute=1 ; minute<=simLength ; minute++ ) {
        ...
        switch(rand() % 4) {
			...
			case 2 : case 3 : numArrivals = 2; break;
		...
		if ( minute - customerQ.front().arrivalTime <= 5 )
			secondOpen = false;	
		else if ( minute - customerQ.front().arrivalTime >= 10)
			secondOpen = true;


        // Try to get next customer if register 2 has no customer.
        if( secondOpen && ! register2.hasCustomer ) {
